...
function conky_main ()
  ...
  themes.custom.dial_bg = {0x3b, 0x42, 0x52, 0.75}
  themes.custom.arc_bg  = {0xd8, 0xde, 0xe9, 0.91}
  themes.custom.cpu_fg  = {{0x61, 0xb1, 0xbf, 1}}
  themes.custom.mem_fg  = {{0xb4, 0x8e, 0xad, 1}, {0x5e, 0x81, 0xac, 1}}
  themes.custom.dsk_fg  = {{0xa3, 0xbe, 0x8c, 1}, {0x4c, 0x56, 0x6a, 1}}
  themes.custom.net_fg  = {{0x81, 0xa1, 0xc1, 1}, {0x5e, 0x81, 0xac, 1}}
  themes.custom.bat_fg  = {{0xd0, 0x87, 0x70, 1}}
  
  curr_theme = themes.custom
  
  -- x values are: 55, 165, 275, 385 
  draw_dial(cr, conky_parse ("${fs_used_perc /}"), "Disk (/)",
                convert_colors(curr_theme.dial_bg), 
                convert_colors(curr_theme.arc_bg), 
                convert_colors(curr_theme.dsk_fg[1]), 55, 190, 43)
  
  draw_dial(cr, conky_parse ("${battery_percent}"), "Battery",
                convert_colors(curr_theme.dial_bg), 
                convert_colors(curr_theme.arc_bg), 
                convert_colors(curr_theme.bat_fg[1]), 165, 190, 43)
  
  draw_dial(cr, conky_parse ("${memperc}"), "Memory",
                convert_colors(curr_theme.dial_bg), 
                convert_colors(curr_theme.arc_bg), 
                convert_colors(curr_theme.mem_fg[1]), 275, 190, 43)

  draw_dial(cr, conky_parse ("${cpu}"), "CPU",
                convert_colors(curr_theme.dial_bg), 
                convert_colors(curr_theme.arc_bg), 
                convert_colors(curr_theme.cpu_fg[1]), 385, 190, 43)
